...
# Definir y codificar las variables categóricas
label_encoders = {}
categorical_columns = ['EDAD', 'SEXO', 'TIPOURGENCIA', 'AFECPRIN', 'AFEC',
...
def get_most_common_medications(cluster):
    ...
    most_common_medications = {col: cluster_data[col].mode().tolist() for col in categorical_columns if col.startswith('MEDICAMENTO')}
...
def predecir_medicamentos(edad, sexo, tipo_urgencia, afeccion_principal, afeccion_secundaria):
    try:
        ...
        # Obtener los medicamentos recomendados para el cluster
        medicamentos_recomendados = medicamentos_por_cluster.get(cluster_pred, {})
        medicamentos = {}
        for med_col in ['MEDICAMENTOS', 'DESCRIPCION']:
            cod_list = medicamentos_recomendados.get(med_col, [])
            medicamentos[med_col] = [label_encoders[med_col].inverse_transform([cod])[0] for cod in cod_list]
    ...
    try:
        ...
        if not (0 <= edad <= 120):
            ...
            for med_col, med_list in medicamentos_pred.items():
                st.write(f"{med_col}: {', '.join(med_list) if med_list else 'No hay datos disponibles'}")
    except ValueError as ve:
